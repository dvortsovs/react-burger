...
function IngredientList({data, type, handleIngredientClick}) {
    return(
        <article>
            ...
            <ul className={`${ingredientListStyle.list} ml-4 mr-4`}>
                {data.map((ingredient) => {
                    if (ingredient.type === type.type) {
                        return <IngredientCard handleIngredientClick={handleIngredientClick} ingredient={ingredient} key={ingredient._id}/>
                    } else return null
                })}
            </ul>
        </article>
